...
- **Four doc types (R13, R13a, R13b, R14, R14a):** W-2, 1099-NEC, 1099-MISC, K-1 discriminated union (K-1 is conditional — see [Known issues](#known-issues--accepted-risks)). Confidence UI: per-field dot, per-doc low-confidence chip on the dashboard, `edited_fields` latch clearing the dot after user save. "Next uncertain" button + Alt+N keyboard shortcut. `needs_review` type-picker. `needs_review` rows excluded from CSV export.
...
- Filled-fixture accuracy calibration (see [Extraction quality](#extraction-quality))
...
| Item                                                                                                                                   | Impact                                                                                                                                      | Status                                                                                                                                                            |
| -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `EXTRACTION_REPORT.md` is a **blank-fixture baseline** — field accuracy here measures schema conformance, not real extraction quality. | Reviewer cannot judge real-world accuracy from the committed report.                                                                        | Filled-fixture curation backlog lives at `fixtures/README.md`. First post-submission backlog item.                                                                |
| **K-1 inclusion decision deferred** — K-1 fixtures are still blank baselines. Current build keeps K-1 in the discriminated union.      | If real K-1 accuracy lands < 80%, K-1 should drop from the union, the CSV, and the export.                                                  | Drop path is mechanical and documented in the parent plan's Risks table.                                                                                          |
...
| ----------- | -------- | -------------- | -------------- |
| `w2`        | 1        | 1/1 (100%)     | 10/10 (100%)   |
| `1099_nec`  | 1        | 1/1 (100%)     | 6/6 (100%)     |
| `1099_misc` | 1        | 1/1 (100%)     | 8/8 (100%)     |
| `k1`        | 1        | 1/1 (100%)     | 8/8 (100%)     |

**Important caveat:** fixtures are blank IRS sample forms. Accuracy here is schema conformance — the model returning the right shape with the right types — not real extraction on filled forms. Filled-fixture curation is the first item in the post-submission backlog at `fixtures/README.md`. The `EXTRACTION_REPORT.md` header carries this caveat prominently.
